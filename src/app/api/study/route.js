...
export async function GET(request) {
  ...
  if (action === 'lessons') {
    const items = db.prepare(`SELECT i.*, a.srs_stage FROM items i JOIN assignments a ON i.id = a.item_id WHERE a.unlocked = 1 AND a.srs_stage = 0 LIMIT 5`).all();
    return NextResponse.json(items.map(parseItem));
  }

  if (action === 'reviews') {
    const items = db.prepare(`SELECT i.*, a.srs_stage FROM items i JOIN assignments a ON i.id = a.item_id WHERE a.srs_stage > 0 AND a.srs_stage < 9 AND a.next_review <= datetime('now') LIMIT 50`).all();
    return NextResponse.json(items.map(parseItem));
  }
