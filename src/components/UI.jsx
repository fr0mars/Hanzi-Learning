...
import { TONE_COLORS, TYPE_COLORS, SECTS } from '@/lib/constants';
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-20 font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-8 animate-in">
        <div className={`relative overflow-hidden rounded-3xl border border-white/10 shadow-2xl p-8 flex flex-col md:flex-row items-center gap-8 bg-gradient-to-r ${userSect ? userSect.bg : 'from-slate-800 to-slate-900'}`}>
          ...
          <div className="relative z-10 text-center md:text-left flex-1">
            <h2 className="text-3xl font-bold text-white mb-2">{user.username} {userSect && <span className="ml-2">{userSect.icon}</span>}</h2>
            <div className="inline-flex items-center gap-2 bg-black/20 px-3 py-1 rounded-full border border-white/10 text-sm text-slate-300 backdrop-blur-md"><span>{userSect ? userSect.name : "Disciple sans secte"}</span></div>
          </div>
